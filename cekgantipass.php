...
if ($data) {
    ...
    if ($password_baru == $konfirmasi_password) {
        ...
        if ($ubah){
            "<script>alert('Password anda berhasil diubah, silahkan login untuk menguji password baru anda.!');document.location='login2.php'</script>";
        }
    } else {
        echo "<script>alert('Maaf, Password Baru & Konfirmasi password yang anda inputkan tidak sama!');document.location='gantipassword.php'</script>";
    }
} else {
    echo "<script>alert('Maaf, Password lama anda tidak sesuai/tidak terdaftar!');document.location='gantipassword.php'</script>";
}